---
title: "control probe intensities"
...
```{r}
outdir <-  "/home/dmartino/Epigenetics_analysis"
```

```{r prepare sample metadata, message= FALSE}
aws.s3::s3load(object = 's3://epichipc-main/Epigenetics/Rawfiles/epicraw-1-15.Rdata')
```

```{r}
#Remove the '_1' from the first technical replicate
metadata$Sample_Name <- str_replace(metadata$Sample_Name, "_1","")
...
ctrl = rbind(as.matrix(BSCI.Green), 
             as.matrix(BSCI.Red), 
             as.matrix(BSCII.Red), 
             (stain.Red), (stain.Green), 
             (extensionA.Red), (extensionT.Red), 
             (extensionC.Green), (extensionG.Green), 
             (hybridH.Green), (hybridM.Green), (hybridL.Green),
             as.matrix(target.Green),
             as.matrix(specI.Green),
             as.matrix(specI.Red), as.matrix(specII.Red),
             (np.Red[1,]),(np.Red[2,]),
             (np.Green[1,]),(np.Green[2,]),
...
pca <- prcomp(na.omit(ctrl))
ctrlprobes.scores = pca$x
colnames(ctrlprobes.scores) = paste(colnames(ctrlprobes.scores), '_cp', sep='')

save(ctrl,ctrlprobes.scores, file = file.path(outdir,"ctrlprobes.RData"))

put_object(
     file = "/home/dmartino/Epigenetics_analysis/ctrlprobes.RData",
     object = "DavidM/ctrlprobes.RData",
